...
from orp.database import K, O, OrpTable, OrpDatabase


class SqlTable(OrpTable[K, O]):
...
class SqlDatabase(OrpDatabase[O]):

    def __init__(self, tables: t.Dict[t.Type[O], SqlTable], engine: Engine):
        self._tables = tables
        self._engine = engine
    ...
    def created_at(self) -> datetime.datetime:
        with self._engine.connect() as connection:
            return connection.execute(select([meta_info.c.created_at]).order_by(meta_info.c.version.desc()).limit(1)).fetchone()[0]

    @property
    def checksum(self) -> bytes:
        with self._engine.connect() as connection:
            return connection.execute(select([meta_info.c.checksum]).order_by(meta_info.c.version.desc()).limit(1)).fetchone()[0]
